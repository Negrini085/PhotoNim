...
proc getAllHitPayload*(handler: ShapeHandler, worldInvRay: Ray): Option[seq[HitPayload]] =
    # Procedure to get all HitPayload, necessary to have CSGDiff and CSGInt
    ...
    of skAABox:
        ...
        if txSpan.min > tySpan.max or tySpan.min > txSpan.max: return none seq[HitPayload]

        let tzSpan = newInterval(min.z / worldInvRay.dir[2], max.z / worldInvRay.dir[2])
        var hitSpan = newInterval(max(txSpan.min, tySpan.min), min(txSpan.max, tySpan.max))

        if hitSpan.min > tzSpan.max or tzSpan.min > hitSpan.max: return none seq[HitPayload]

        if tzSpan.min > hitSpan.min: hitSpan.min = tzSpan.min
        if tzSpan.max < hitSpan.max: hitSpan.max = tzSpan.max

        let tHit = if handler.shape.aabb.contains(worldInvRay.origin): hitSpan.max else: hitSpan.min
        if not worldInvRay.tspan.contains(tHit): return none seq[HitPayload]

        if tHit == hitSpan.max:
            return some @[HitPayload(handler: handler, ray: worldinvRay, t: tHit)]
        
        return some @[
            HitPayload(handler: handler, ray: worldinvRay, t: hitSpan.min),
            HitPayload(handler: handler, ray: worldinvRay, t: hitSpan.max)
            ]
    ...
    of skTriangle:
        ...
        let sol = try: solve(mat, vec) except ValueError: return none seq[HitPayload]
        if not worldInvRay.tspan.contains(sol[2]): return none seq[HitPayload]
        if sol[0] < 0.0 or sol[1] < 0.0 or sol[0] + sol[1] > 1.0: return none seq[HitPayload]
    ...
    of skSphere:
        ...
        let delta_4 = b * b - a * c
        if delta_4 < 0: return none seq[HitPayload]
        ...
        let tHit = 
            ...
            else: return none seq[HitPayload]
        
        if tHit == t_r:
            return some @[HitPayload(handler: handler, ray: worldInvRay, t: tHit)]
        
        return some @[
            HitPayload(handler: handler, ray: worldInvRay, t: t_l),
            HitPayload(handler: handler, ray: worldInvRay, t: t_r)
            ]

    of skPlane:
        if abs(worldInvRay.dir[2]) < epsilon(float32): return none seq[HitPayload]
        let tHit = -worldInvRay.origin.z / worldInvRay.dir[2]
        if not worldInvRay.tspan.contains(t_hit): return none seq[HitPayload]
    ...
    of skCylinder:
        ...
        if delta < 0.0: return none seq[HitPayload]

        var tspan = newInterval((-b - sqrt(delta)) / (2 * a), (-b + sqrt(delta)) / (2 * a))

        if tspan.min > worldInvRay.tspan.max or tspan.max < worldInvRay.tspan.min: return none seq[HitPayload]

        var tHit = tspan.min
        if tHit < worldInvRay.tspan.min:
            if tspan.max > worldInvRay.tspan.max: return none seq[HitPayload]
            tHit = tspan.max
        ...
        if areClose(worldInvRay.dir[0], 0) and areClose(worldInvRay.dir[1], 0) and worldInvRay.dir[2] != 0:
            if not areClose(pow(worldInvRay.origin.x, 2) + pow(worldInvRay.origin.y, 2), pow(handler.shape.R, 2)):
                return none seq[HitPayload]

        if hitPt.z < handler.shape.zSpan.min or hitPt.z > handler.shape.zSpan.max or phi > handler.shape.phiMax:
            if tHit == tspan.max: return none seq[HitPayload]
            tHit = tspan.max
            if tHit > worldInvRay.tspan.max: return none seq[HitPayload]
            
            hitPt = worldInvRay.at(tHit)
            phi = arctan2(hitPt.y, hitPt.x)
            if phi < 0.0: phi += 2.0 * PI
            if hitPt.z < handler.shape.zSpan.min or hitPt.z > handler.shape.zSpan.max or phi > handler.shape.phiMax: return none seq[HitPayload]

        if tHit == tspan.max:
            return some @[HitPayload(handler: handler, ray: worldInvRay, t: tHit)]
        
        return some @[
            HitPayload(handler: handler, ray: worldInvRay, t: tspan.min),
            HitPayload(handler: handler, ray: worldInvRay, t: tspan.max)
            ]
    ...
    of skEllipsoid: 
        ...
        if hit.isNone: return none seq[HitPayload]
        elif hit.get.len == 1:
            return some @[HitPayload(handler: handler, ray: worldInvRay, t: hit.get[0].t)]
        
        return some @[
            HitPayload(handler: handler, ray: worldInvRay, t: hit.get[0].t),
            HitPayload(handler: handler, ray: worldInvRay, t: hit.get[1].t)
            ]
