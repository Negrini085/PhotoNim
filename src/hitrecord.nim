...
proc newHitPayload*(refSystem: ReferenceSystem, ray: Ray, handler: ShapeHandler): Option[HitPayload] =
    ...
    of skAABox:
        ...
        return some HitPayload(handler: handler, ray: ray, t: tHit)
    ...
    of skTriangle:
        ...
        return some HitPayload(handler: handler, ray: ray, t: sol[2])
...
proc getHitPayloads(subScene: SubScene; ray: Ray): seq[HitPayload] =
    assert subScene.tree.kind == nkLeaf, "Cannot get an HitPayload from a SceneNode of kind different from nkLeaf."

    var hittedHandlers: seq[ShapeHandler]
    for handler in subScene.tree.handlers:
        if checkIntersection(subScene.rs.getLocalAABB(handler), ray): hittedHandlers.add handler
...
proc newHitRecord*(refSystem: ReferenceSystem, hitLeafs: seq[SceneNode], ray: Ray): Option[seq[HitPayload]] =
    let hitPayloads = hitLeafs
        .mapIt(getHitPayloads((refSystem, it), ray))
        .filterIt(it.len > 0)
